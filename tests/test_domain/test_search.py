...
class TestSearchAll:
    ...
    async def test_finds_contacts_by_name(self, db, make_contact):
        ...
        results = await search_queries.search_all(db, "sam", TENANT_ID)
        assert len(results["contacts"]) >= 1

    async def test_finds_interactions_by_summary(
        self, db, make_contact, make_interaction
    ):
        contact = await make_contact(name="Interactor")
        await make_interaction(contact.id, summary="Discussed quantum computing")

        results = await search_queries.search_all(db, "quantum", TENANT_ID)
        assert len(results["interactions"]) >= 1

    async def test_empty_query_returns_empty(self, db):
        results = await search_queries.search_all(db, "  ", TENANT_ID)
...
class TestCategoryFilter:
    ...
    async def test_filter_by_category(self, db, make_contact):
        ...
        investors = await search_queries.get_contacts_by_category(
            db, "investor", TENANT_ID
        )
...
class TestCompanyInteractions:
    ...
    async def test_find_interactions_by_company(
        self, db, make_contact, make_interaction
    ):
        ...
        results = await search_queries.get_interactions_by_company(
            db, "BigCorp", TENANT_ID
        )
...
class TestRecentActivity:
    ...
    async def test_returns_recent_only(self, db, make_contact, make_interaction):
        ...
        results = await search_queries.get_recent_activity(db, TENANT_ID, days=7)
...
class TestContactWithInteractions:
    ...
    async def test_returns_contact_and_interactions(
        self, db, make_contact, make_interaction
    ):
        ...
        result = await search_queries.get_contact_with_interactions(
